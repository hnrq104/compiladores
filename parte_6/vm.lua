...
local function read_line(line)
    ...
    if is_push_str then
        ...
        return makeInst("PUSH_STRING", { prepare_string(second_tok) })
    end
...
-- a prog is { Insts = instructions, LabelSet = label_set }
local function assemble_program(prog)
    for i = 1, #prog.Insts do
        local inst = prog.Insts[i]
        if is_jmp(inst[1]) then
            inst[2][1] = prog.LabelSet[inst[2][1]] -- CHANGE LABEL TO INST_NUMBER
        end
        -- if call do similar to function
    end
    return prog.Insts
end
