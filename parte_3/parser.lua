...
-- reads a hexa number WITH the 0x on front
local function readHexaNumber(ls)
    ...
    local first_dot = true -- haven't seen a dot yet
    local is_hexa_complete = false -- the number isn't of the form 0x. 
    ...
    makeTok("NUMBER", tonumber(n), start_l, start_c, ls.currentLine, ls.currentColumn)
...
local function readNumber(ls)
    ...
    --[[
        ...
        return makeTok(tag, tonumber(n), start_l, start_c, ls.currentLine, ls.currentColumn)
    ]]
...
local PS = { -- stands for Parser State
    ...
    nextNextToken = getToken(LS)
}


local function advanceParser(ps)
    ps.nextToken = ps.nextNextToken
    ps.nextNextToken = getToken(LS)
end
...
local function parseExponentExp(ps)
    local e = parseSimples(ps)
    if ps.nextToken.Tag == "^" then
        advanceParser(ps)
        return makeBinop("^",e,parseUnopExp(ps))
    end
    return e
end
